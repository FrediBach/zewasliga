...
test("holding Shift shows a detail loupe at the pointer", async () => {
  ...
  assert.match(page, /event\.key === "\+" \|\| event\.code === "NumpadAdd"/);
  assert.match(page, /Math\.min\(DETAIL_LENS_MAX_ZOOM, zoom \+ DETAIL_LENS_ZOOM_STEP\)/);
  assert.match(page, /transform: `scale\(\$\{detailLensZoom\}\)`/);
