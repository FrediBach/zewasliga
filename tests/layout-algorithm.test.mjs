...
import { arrangePhotos, createSmartMosaic } from "../src/layout.ts";
...
test("smart mosaic covers a landscape viewport with very little crop", () => {
  const layout = createSmartMosaic(photos, { width: 1920, height: 1080 }, new Map(), new Set(), 0, seededRandom(42));
  assert.ok(layout);
...
test("the same selection reflows to fill a portrait viewport", () => {
  const layout = arrangePhotos(photos, { width: 390, height: 844 }, seededRandom(84));
  assert.ok(layout);
  assertExactCoverage(layout);
